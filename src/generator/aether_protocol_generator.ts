...
const currentFields = this.getFieldTypes(cfg?.fields || {});
        ...
        sb.push(`        result.add('${simpleClassName}(');`); // <-- ИЗМЕНЕНО
        let isFirstField = true;
        allFields.forEach((_, fieldName) => {
            if (!isFirstField) sb.push(`        result.add(', ');`); // <-- ИЗМЕНЕНО
            sb.push(`        result.add('${fieldName}:').add(this.${fieldName});`); // <-- ИЗМЕНЕНО
            isFirstField = false;
        });
        ...
        allConstants.forEach((constInfo, constName) => {
            ...
            sb.push(`        result.add('${constName}:').add(this.${constInfo.getGetterName()}());`); // <-- ИЗМЕНЕНО
            isFirstField = false;
        });

        sb.push(`        result.add(')');`); // <-- ИЗМЕНЕНО
    ...
    private generateStreamClass(name: string, cfg: TypeDefinition): string {
        ...
        sb.push(`    public toString(result: AString): AString { result.add('${name}(').add('data:').add(this.data).add(')'); return result; }`); // <-- ИЗМЕНЕНО
