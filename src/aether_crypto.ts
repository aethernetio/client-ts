...
// =============================================================================================
// CRYPTO INTERFACES & IMPLEMENTATION
// =============================================================================================
...
export type DtoPairKeysAsymSigned = {
    getKey(): { getKey(): DtoKey, getSign(): Sign },
...
export interface CryptoProvider {
    ...
    // *** ИСПРАВЛЕНО: Public идет первым, Private вторым ***
...
export const CryptoProviderFactory = {
    register: (provider: CryptoProvider): void => {
        ...
        if (providers.has(nameLower)) {
            Log.warn("CryptoProvider already registered: $name. Overwriting.", { name: nameLower });
        }
        providers.set(nameLower, provider);
        Log.debug("Registered CryptoProvider: $name", { name: provider.getCryptoLibName() });
    },
    ...
    makeSigners: (): Set<Signer> => {
        ...
        for (const p of CryptoProviderFactory.all()) {
            try {
                ...
                Log.warn("Skipping crypto provider $pname for signer creation: $err", {
